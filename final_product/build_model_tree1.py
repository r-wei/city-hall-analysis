...
vectorizer = TfidfVectorizer(analyzer='word', max_df=0.4, #max_features=5,
                                 min_df=0.002, stop_words=set(my_stop_words)
                                 #use_idf=opts.use_idf
				)#still produces some really big non-meaningful groups
...
clf_forest = ensemble.RandomForestClassifier() #could pick different classifier

steps = [('feature_selection', selector), ('random_forest', clf_forest)]

pipeline = pipeline.Pipeline(steps)
 
t0 = time()
X_train, X_test, y_train, y_test = cross_validation.train_test_split(X_matrix, label_dum, test_size=0.33, random_state=42)
...
print("Tree building done in: %fs" % (time()-t0))
print(report)
