#Try to find misspellings using rare words identified by the correlation matrix. (Here, rare words = keywords)
#We're using the same ideas as in grouping_docs.py except now we're looking for rare rather than common words

#Results weren't promising, even when we cleaned the words first 
#There are too many words that appear in only ONE title
#This suggests that we should try this code on the entire text of the documents. see find_misspellings_corpus.py for that implementation
